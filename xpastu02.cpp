...
float maxHoverTime = 2.f; // maximální doba, po kterou kamera může zůstat ve vzduchu
float hoverTime = 0.0f; // aktuální doba, po kterou kamera zůstává ve vzduchu

//creative
bool creative = false;

// Osvětlení
GLfloat light_position[] = { 0.0, 10.0, 0.0, 10.0 }; // umístění světla
GLfloat light_ambient[] = { 0.8, 0.8, 0.2, 1.0 }; // ambientní barva světla
GLfloat light_diffuse[] = { 1., 1.0, 1.0, 0.5 }; // difuzní barva světla
bool svetlo_je = false; // Osvětlení
int color_index = 0;

// text v rohu
char last_command[50] = "Stisknete libovolnou klavesu"; // Text posledního příkazu uživatelského rozhraní
bool draw_text = true; // Příznak pro vykreslování textu
char ball_text[200] = "Pozice koule: Chybi";

// camera a míč pro vyhození po balistice
...
{
	// Aktualizace globálních proměnných s novými rozměry okna
	windowWidth = w;
	windowHeight = h;
...
// Funkce pro načtení textury a nastavení parametrů texturování
void loadTexture() {
...
void DrawPlane(int size)
{
	...
	if (svetlo_je)
	{
		glLightfv(GL_LIGHT0, GL_POSITION, light_position); // nastavení pozice světla
		glLightfv(GL_LIGHT0, GL_AMBIENT, light_ambient); // nastavení ambientní barvy světla
		glLightfv(GL_LIGHT0, GL_DIFFUSE, light_diffuse); // nastavení difuzní barvy světla
		glEnable(GL_LIGHT0); // zapnutí světla

		glEnable(GL_LIGHTING); // zapnutí osvětlení
...
void checkCollisionWithCube(float cubeX, float cubeY, float cubeZ, float cubeSize, float& cameraX, float& cameraY, float& cameraZ) {
	...
	// Pokud je kamera uvnitř krychle, vrátit ji na předchozí platnou pozici
	if (tranx < -cubeX + CubeSize && tranx > - cubeX - CubeSize &&
		...
		// Vrácení na předchozí platnou pozici
...
void DrawCube(float x, float y, float z, float size, bool move) {
	...
	GLfloat rgba[] = { 1.0f, 1.0f, 1.0f, 0.8f }; // bílá barva s 50% průhledností
...
void renderBezierSurface(float x, float y, float z) {
	...
	glPushMatrix(); // Uložení transformační matice
	...
	GLuint textureID; // Deklarace proměnné pro identifikátor textury
	...
	// Vygenerování různobarevného plátna
	for (u = 0; u < 30; u++) {
		glBegin(GL_TRIANGLE_STRIP);
		for (v = 0; v <= 30; v++) {
			// Výpočet barevného přechodu
			GLfloat color[4];
			color[0] = u / 30.0;
			color[1] = v / 30.0;
			color[2] = 1.0 - (u / 30.0);
			...
			if(texture_is)
			{
			// Výpočet texturovacích souřadnic
	...
	glPopMatrix(); // Obnovení transformační matice
}
...
void OnDisplay(void)				   
{
	...
	// Bezieruv Plát
	int numObjects = 50; // Počet objektů
	float radius = (150.0 + xOffset) /3.; // Poloměr kruhu
	glRotatef(rOffset, 0., 1., 0.);
	for (int i = 0; i < numObjects; i++) {
		float angle = (2.0 * PI * i) / numObjects; // Úhel pro umístění objektu na kružnici
		float x = radius * cos(angle); // Výpočet souřadnice x
		float y = 0.0; // Souřadnice y (v tomto případě zůstává konstantní)
		float z = radius * sin(angle); // Výpočet souřadnice z
...
{ 
	...
	// rozdeleni obrazovky na 4 části, kde se klikne
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		// Získání normalizovaných souřadnic myši
		float normalizedX = static_cast<float>(x) / windowWidth;
		float normalizedY = static_cast<float>(y) / windowHeight;

		// Zjištění do které části obrazovky bylo kliknuto
...
void OnSpecial(int key, int mx, int my)
{
	...
	if (!creative) {
		...
		case GLUT_KEY_LEFT:
		case GLUT_KEY_RIGHT:
			if (trany < maxHeight) // pokud kamera není příliš vysoko, pohybuje se nahoru
			{
				trany += cameraSpeed;
				hoverTime = 0.0f; // resetuje dobu zůstávání ve vzduchu
			}
			else // pokud dosáhla maximální výšky, začne se snižovat
			{
				trany -= cameraSpeed;
			}
			break;
		...
		// pokud kamera zůstává ve vzduchu příliš dlouho, spadne dolů
		if (trany > minHeight) // kamera je nahoře
		{
			hoverTime += 1.f; // zvýší dobu zůstávání ve vzduchu
			if (hoverTime >= maxHoverTime) // pokud je doba překročena, spadne dolů
			{
				trany -= cameraSpeed;
				hoverTime = 0.0f; // resetuje dobu zůstávání ve vzduchu
			}
...
int main(int argc, char* argv[])
{
	...
	// spuštění časovače pro změnu ambientního světla
